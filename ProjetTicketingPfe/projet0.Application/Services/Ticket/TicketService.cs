    ...
    public class TicketService : ITicketService
    {
        ...
        private Expression<Func<TicketEntity, bool>>? BuildFilter(TicketPagedRequest request)
        {
            ...
            if (request.DateLimiteStatut.HasValue)
            {
                ...
                switch (request.DateLimiteStatut.Value)
                {
                    case DateLimiteStatut.Expire:
                        // Tickets non résolus avec date limite dépassée
                        predicates.Add(t => t.StatutTicket != StatutTicket.Resolu &&
                                            t.DateLimite != DateTime.MinValue &&
                                            t.DateLimite < today);
                        break;

                    case DateLimiteStatut.RealiseAvantDelai:
                        // Tickets résolus avant la date limite
                        predicates.Add(t => t.StatutTicket == StatutTicket.Resolu &&
                                            t.DateCloture.HasValue &&
                                            t.DateLimite != DateTime.MinValue &&
                                            t.DateCloture.Value <= t.DateLimite);
                        break;

                    case DateLimiteStatut.RealiseApresDelai:
                        // Tickets résolus après la date limite
                        predicates.Add(t => t.StatutTicket == StatutTicket.Resolu &&
                                            t.DateCloture.HasValue &&
                                            t.DateLimite != DateTime.MinValue &&
                                            t.DateCloture.Value > t.DateLimite);
                    ...
                    case DateLimiteStatut.JoursRestants:
                        // Tickets non résolus avec date limite dans le futur
                        predicates.Add(t => t.StatutTicket != StatutTicket.Resolu &&
                                            t.DateLimite != DateTime.MinValue &&
                                            t.DateLimite >= today);
                        break;
        ...
        public async Task<ApiResponse<UpdateTicketResponseDTO>> UpdateTicketAsync(Guid id, UpdateTicketDTO dto, Guid userId)
        {
            return await MeasureAsync(nameof(UpdateTicketAsync), new { id, dto }, async () =>
            {
                ...
                try
                {
                    ...
                    if (ancienAssigneeId.HasValue)
                    {
                        var ancienTech = await _userRepository.GetByIdAsync(ancienAssigneeId.Value);
                        ancienAssigneeNom = ancienTech != null ? $"{ancienTech.Prenom} {ancienTech.Nom}" : null;
                    }
                    ...
                    if (isAdmin)
                    {
                        ...
                        if (dto.DateLimite != ticket.DateLimite)
                        {
                            ...
                            modifications.Add($"Date limite : {dto.DateLimite:dd/MM/yyyy HH:mm}");
                        }
                    }
                        ...
                        if (dto.AssigneeId.HasValue)
                        {
                            ...
                            else
                            {
                                ...
                                else
                                {
                                    ...
                                    {
                                        peutAssigner = (ticket.AssigneeId == userId && ticket.StatutTicket == StatutTicket.EnCours);
                                        if (!peutAssigner)
                                        {
                                    ...
                                    if (peutAssigner)
                                    {
                                        nouveauTechnicienNom = $"{assignee.Prenom} {assignee.Nom}";
                                        var statutAvantAssignation = ticket.StatutTicket;
                                        ...
                                        {
                                            estResteEnCours = true;
                                            modifications.Add("Statut reste En cours");
                                            _logger.LogWarning("Ticket réassigné - Statut maintenu à En cours");
                                        }
                                        else if (statutAvantAssignation != StatutTicket.Resolu)
                                        {
                                            ticket.StatutTicket = StatutTicket.Assigne;
                                            modifications.Add("Statut -> Assigné");
                                            _logger.LogWarning("Ticket réassigné - Statut changé de {Ancien} à Assigné", statutAvantAssignation);
                                        }
                                    }
                                }
                            }
                    ...
                    if (isTechnicien && dto.StatutTicket.HasValue && dto.StatutTicket.Value != ticket.StatutTicket)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                erreurs.Add($"Transition interdite: '{ticket.StatutTicket}' -> '{nouveauStatut}'");
                            }
                    ...
                    if (modifications.Any())
                    {
                        ...
                        catch (DbUpdateConcurrencyException)
                        {
                            ...
                            {
                                return ApiResponse<UpdateTicketResponseDTO>.Failure("Le ticket a été modifié à plusieurs reprises. Veuillez réessayer.");
                            }
                        }
                    }
                    // ========================================================
                    ...
                    if (modifications.Any() && sauvegardeReussie)
                    {
                        ...
                        string modificationsTexte = string.Join(", ", modifications);
                        modificationsTexte = modificationsTexte.Replace("Assignation ->", "Assignation →");

                        TypeNotification typeNotif;
                        string titreNotification;
                        string messagePourTechnicien;
                        string messagePourAdmins;

                        // Cas 1: Réassignation d'un ticket
                        if (aEuAssignation)
                        {
                            typeNotif = TypeNotification.TicketModifie;
                            titreNotification = $"Ticket {ticket.ReferenceTicket} - Réassigné";
                            ...
                            // Notifier le NOUVEAU TECHNICIEN assigné
                        ...
                        {
                            typeNotif = TypeNotification.TicketModifie;
                            titreNotification = $"Ticket {ticket.ReferenceTicket} - Modifié";

                            messagePourTechnicien = $"{actionneur} a modifié le ticket qui vous est assigné. Modifications: {modificationsTexte}";
                            messagePourAdmins = $"{actionneur} a modifié le ticket '{ticket.TitreTicket}'. Modifications: {modificationsTexte}";
...
if (ticket.IncidentTickets != null && ticket.IncidentTickets.Any())
{
    foreach (var lien in ticket.IncidentTickets)
    {
        ...
        incident = await _incidentRepository.GetByIdAsync(lien.IncidentId);
        ...
        if (incident != null && incident.CreatedById.HasValue)
        {
            ...
            if (vientDEtreResolu && nouveauStatut == StatutTicket.Resolu)
            {
                var createurIncident = await _userRepository.GetByIdAsync(incident.CreatedById.Value);
                if (createurIncident != null)
                {
                    string messageCommercant = $"L'incident '{incident.CodeIncident}' est maintenant résolu.";
                    
                    await _notificationService.CreateIncidentNotificationAsync(
                        createurIncident.Id,
                        incident.Id,
                        TypeNotification.IncidentResolu,
                        $"Incident {incident.CodeIncident} - Résolu",
                        messageCommercant
                    );
                    
                    _logger.LogInformation("Notification envoyée au commerçant {CommercantId} pour l'incident {IncidentId} (était {AncienStatut} -> {NouveauStatut})",
                        createurIncident.Id, incident.Id, etaitDejaFerme ? "Fermé" : "Non Fermé", incident.StatutIncident);
                }
            }
            else if (etaitDejaFerme)
            {
                _logger.LogDebug("Incident {IncidentId} déjà fermé - pas de notification envoyée", incident.Id);
            }
        }
    }
    modifications.Add("Incidents mis à jour");
        ...
        public async Task<ApiResponse<TicketDashboardDTO>> GetTicketDashboardAsync()
        {
            return await MeasureAsync(nameof(GetTicketDashboardAsync), null, async () =>
            {
                try
                {
                    ...
                    var archivedTickets = allTicketsList
                        .Where(t => allArchivedTicketIds.Contains(t.Id))
                        .ToList();
                    ...
                    // ============================================
                    // 1. STATISTIQUES GLOBALES (sur tickets NON archivés)
                    // ============================================
        ...
        public async Task<ApiResponse<TicketTechnicienDashboardDTO>> GetTechnicienDashboardAsync(Guid technicienId)
        {
            return await MeasureAsync(nameof(GetTechnicienDashboardAsync), null, async () =>
            {
                try
                {
                    _logger.LogInformation("Récupération du dashboard pour le technicien {TechnicienId}", technicienId);

                    // Récupérer les IDs des tickets archivés par ce technicien
                    var archivedTicketIds = await _ticketArchiveRepository
                        .GetArchivedTicketIdsByUserAsync(technicienId);

                    // Récupérer tous les tickets assignés au technicien
                    var query = _ticketRepository.GetFilteredQuery(t => t.AssigneeId == technicienId);
                    var ticketsList = await query.ToListAsync();

                    // Filtrer les tickets non archivés et archivés
                    var ticketsNonArchives = ticketsList.Where(t => !archivedTicketIds.Contains(t.Id)).ToList();
                    var ticketsArchives = ticketsList.Where(t => archivedTicketIds.Contains(t.Id)).ToList();

                    // ============================================
                    // 1. STATISTIQUES GLOBALES
                    // ============================================
                    var total = ticketsList.Count;
                    var assignes = ticketsNonArchives.Count(t => t.StatutTicket == StatutTicket.Assigne);
                    var enCours = ticketsNonArchives.Count(t => t.StatutTicket == StatutTicket.EnCours);
                    var resolus = ticketsNonArchives.Count(t => t.StatutTicket == StatutTicket.Resolu);
                    var nonArchiveCount = ticketsNonArchives.Count;
                    var archiveCount = ticketsArchives.Count;

                    var overview = new TicketTechnicienOverviewDTO
                    {
                        TotalTickets = total,
                        TicketsAssignes = assignes,
                        TicketsEnCours = enCours,
                        TicketsResolus = resolus,
                        TicketsNonArchive = nonArchiveCount,
                        TicketsArchives = archiveCount,

                        TauxAssignes = total > 0 ? Math.Round((double)assignes / total * 100, 1) : 0,
                        TauxEnCours = total > 0 ? Math.Round((double)enCours / total * 100, 1) : 0,
                        TauxResolus = total > 0 ? Math.Round((double)resolus / total * 100, 1) : 0,
                        TauxResolution = nonArchiveCount > 0 ? Math.Round((double)resolus / nonArchiveCount * 100, 1) : 0
                    };
                    ...
                    var statsParStatut = new List<TicketStatutStatDTO>
            {
                ...
                new TicketStatutStatDTO
                {
                    ...
                    Count = archiveCount,
                    Color = "#6c757d",
                    Pourcentage = total > 0 ? Math.Round((double)archiveCount / total * 100, 1) : 0
                }
            };
                    ...
                    {
                        var date = today.AddDays(-i);
                        var ticketsDuJour = ticketsNonArchives
                            .Where(t => t.DateCreation.Date == date)
                            .ToList();
                    ...
                    // ============================================
                    // 4. STATISTIQUES DE RÉSOLUTION
                    // ============================================
                    var ticketsResolusList = ticketsNonArchives
                        .Where(t => t.StatutTicket == StatutTicket.Resolu && t.DateCloture.HasValue)
                        .ToList();

                    var statsResolution = new List<TicketTechnicienResolutionDTO>();

                    // Résolution par jour (derniers 7 jours)
                    for (int i = 6; i >= 0; i--)
                    {
                        var date = today.AddDays(-i);
                        var ticketsResolusJour = ticketsResolusList
                            .Where(t => t.DateCloture.Value.Date == date)
                            .ToList();

                        var tempsResolution = ticketsResolusJour
                            .Select(t => (t.DateCloture.Value - t.DateCreation).TotalHours)
                            .ToList();
                        ...
                        statsResolution.Add(new TicketTechnicienResolutionDTO
                        {
                            Periode = "Jour",
                            Date = date,
                            TicketsResolus = ticketsResolusJour.Count,
                            TempsMoyenResolutionHeures = moyenneHeures,
                            TempsMoyenResolutionJours = moyenneJours
                        });
                    }
                    ...
                    var weeksCount = 4; // Dernières 4 semaines
                    ...
                    for (int weekNumber = weeksCount; weekNumber >= 1; weekNumber--)
                    {
                        ...
                        var ticketsSemaine = ticketsNonArchives
                            .Where(t => t.DateCreation.Date >= debutSemaine && t.DateCreation.Date <= finSemaine)
                            .ToList();
                    ...
                    for (int i = 5; i >= 0; i--)
                    {
                        ...
                        var ticketsMois = ticketsNonArchives
                            .Where(t => t.DateCreation.Date >= debutMois && t.DateCreation.Date <= finMois)
                            .ToList();
                    ...
                    var dashboard = new TicketTechnicienDashboardDTO
                    {
                        Overview = overview,
                        StatsParStatut = statsParStatut,
                        StatsParJour = statsParJour,
                    ...
                    _logger.LogInformation("Dashboard technicien généré - Total: {Total}, Résolus: {Resolus}, Archivés: {Archives}",
                        total, resolus, archiveCount);

                    return ApiResponse<TicketTechnicienDashboardDTO>.Success(dashboard);
                }
                catch (Exception ex)
                {
                    _logger.LogError(ex, "Erreur lors de la génération du dashboard pour le technicien");
                    return ApiResponse<TicketTechnicienDashboardDTO>.Failure("Erreur interne du serveur");
                }
            });
        }
